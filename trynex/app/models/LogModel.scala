...
import models.UserSystemInfoLogin
...
object LogModel {
  ...
  def UserSysDetFromRequest(request: RequestHeader) = {

    val browserDetails: String = (request.headers.get("User-Agent")).toString()
    val userAgent: String = browserDetails
    val user: String = userAgent.toLowerCase()
    var os: String = ""
    var browser: String = ""
    Logger.info("User Agent for the request is===>" + browserDetails)
    os =
      if (userAgent.toLowerCase().indexOf("windows") >= 0) "Windows"
      else if (userAgent.toLowerCase().indexOf("mac") >= 0) "Mac"
      else if (userAgent.toLowerCase().indexOf("x11") >= 0) "Unix"
      else if (userAgent.toLowerCase().indexOf("android") >= 0) "Android"
      else if (userAgent.toLowerCase().indexOf("iphone") >= 0) "IPhone"
      else "UnKnown, More-Info: " + userAgent
    //===============Browser===========================
    if (user.contains("msie")) {
      ...
      browser = (userAgent
        ...
        "-" +
        (userAgent
          .substring(userAgent.indexOf("Version"))
          .split(" ")(0))
        .split("/")(1)
    } else if (user.contains("opr") || user.contains("opera")) {
      if (user.contains("opera"))
        browser = (userAgent
          .substring(userAgent.indexOf("Opera"))
          .split(" ")(0))
          .split("/")(0) +
          "-" +
          (userAgent
        ...
        browser = ((userAgent
          .substring(userAgent.indexOf("OPR"))
          .split(" ")(0))
          .replace("/", "-"))
          .replace("OPR", "Opera")
    } else
      browser =
        if (user.contains("chrome")) (userAgent
          .substring(userAgent.indexOf("Chrome"))
          .split(" ")(0))
          .replace("/", "-")
        else if ((user.indexOf("mozilla/7.0") > -1) || (user.indexOf(
          "netscape6") != -1) ||
          (user.indexOf("mozilla/4.7") != -1) ||
          (user.indexOf("mozilla/4.78") != -1) ||
          (user.indexOf("mozilla/4.08") != -1) ||
          (user.indexOf("mozilla/3") != -1)) "Netscape-?"
        else if (user.contains("firefox")) (userAgent
          .substring(userAgent.indexOf("Firefox"))
          .split(" ")(0))
          .replace("/", "-")
        else if (user.contains("rv"))
          "IE-" +
            user.substring(user.indexOf("rv") + 3, user.indexOf(")"))
        else "UnKnown, More-Info: " + userAgent
    Logger.info("Operating System======>" + os)
    Logger.info("Browser Name==========>" + browser)
    var userInfo = new models.UserSystemInfoLogin(os, browser)
    userInfo
  }
