...
get_unconfirmed_withdrawal_tx (
  ...
  select id, tx_hash from withdrawals_crypto_tx where
 confirmed is NULL and tx_hash is NOT NULL and currency =a_currency;
...
create or replace function
get_userid_for_rawtransaction (
  a_tx_id bigint,
  out user_id bigint
)
  returns bigint as $$  select user_id from withdrawals where id = (select id from withdrawals_crypto where withdrawals_crypto_tx_id=a_tx_id)
