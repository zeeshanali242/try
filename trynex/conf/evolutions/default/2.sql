...
begin
 ...
 insert into withdrawals(amount, user_id, currency, fee,remarks)
   values (a_amount, a_uid, a_currency, a_fee,a_remarks)
...
create_user_complete (
  a_email varchar(256),
  a_first_name varchar(256),
  a_last_name varchar(256),
  a_mobile_number varchar(256),
  a_password text,
...
begin
  ...
  return create_user(a_email, a_first_name ,a_last_name , a_mobile_number,a_password, a_onMailingList, a_pgp);;
end;;
$$ language plpgsql volatile security definer set search_path = public, pg_temp cost 100;
...
last_24_price (
  ...
  out price numeric(23,8)
    ...
    select price from matches where created <  now() - INTERVAL '1 DAY' and base =a_base and counter=a_counter order by created desc limit 1;
...
declare
  ...
  matched boolean;;
begin
  select * into strict u from users where lower(email) = lower(a_email) or mobile_number = a_email;;


  if user_sms_otp_check(u.id, a_otp) then
    perform new_log(u.id, a_browser_headers, a_email, null, null, a_ip, 'login_success', a_os, a_browser);;
    return u;;
  else
    perform new_log(u.id, a_browser_headers, a_email, null, null, a_ip, 'login_failure', a_os, a_browser);;
    return null;;
  end if;;
end;;
...
begin
  ...
  success = false;;
        ...
        IF EXISTS (select * from mobile_otps where  otp=a_otp and user_id=a_uid and expire >current_timestamp) THEN
        success = true;
      END IF;
...
user_has_totp (
  ...
  select tfa_enabled from users where lower(email) = lower(a_email) or u.mobile_number = a_email;;
$$ language sql volatile security definer set search_path = public, pg_temp cost 100;
...
totp_login_step1 (
  ...
  a_ip inet,
   a_os varchar(256),
  a_browser varchar(256),
) returns text as $$
declare
  u users%rowtype;;
  sec text;;
begin
  select * into u from find_user_by_email_and_password_invoker(a_email, a_password, a_browser_headers, a_ip, true, a_os, a_browser);;
  if u is null then
    return null;;
  end if;;
...
begin
  ...
  return find_user_by_email_and_password_invoker(a_email, a_password, a_browser_headers, a_ip, false ,a_os, a_browser);;
end;;
$$ language plpgsql volatile security definer set search_path = public, pg_temp cost 100;
...
begin
  select u.id, u.active, p.password into u_id, u_active, u_pass from users u
    ...
    where lower(u.email) = lower(a_email) or u.mobile_number = a_email
    order by p.created desc
    limit 1;;

  if not found then
    perform new_log(null, a_browser_headers, a_email, null, null, a_ip, 'login_failure', a_os, a_browser);;
    return null;;
  end if;;

  if u_active and u_pass = crypt(a_password, u_pass) then
    if a_totp_step1 then
      perform new_log(u_id, a_browser_headers, a_email, null, null, a_ip, 'login_partial_success', a_os, a_browser);;
    else
      perform new_log(u_id, a_browser_headers, a_email, null, null, a_ip, 'login_success', a_os, a_browser);;
    end if;;
  ...
  perform new_log(u_id, a_browser_headers, a_email, null, null, a_ip, 'login_failure', a_os, a_browser);;
  return null;;
end;;
$$ language plpgsql volatile security invoker set search_path = public, pg_temp cost 100;
...
find_token (
  ...
  select token, email, creation, expiration, is_signup, first_name, last_name, mobile_number from tokens where token = a_token;;
...
begin
  ...
  insert into event_log (user_id, email, ip, browser_headers, browser_id, ssl_info, type, user_os, user_browser)
  values (a_uid, a_email, a_ip, a_browser_headers, a_browser_id, a_ssl_info, a_type, a_os, a_browser);;
  return;;
end;;
$$ language plpgsql volatile security definer set search_path = public, pg_temp cost 100;
...
begin
  ...
  return query select o.id, o.is_bid, o.remains as amount, o.price,  o.created from orders o
  where user_id = a_user_id and closed = false and o.remains > 0 and counter=a_counter and base=a_base 
...
volume_sum (
  ...
  out volume numeric(23,8)
   ...
   select base,sum(amount) from matches where created >= (current_timestamp - interval '24 hours' ) group by base;
$$ language sql stable security definer set search_path = public, pg_temp cost 100;
...
begin
  ...
  return query select th.id, th.amount, th.created, th.price, th.base, th.counter, th.is_bid, th.fee from (
    (
      select m.id, m.amount, m.created, m.price, m.base, m.counter, true as is_bid, m.bid_fee as fee
    ...
    union all
    (
      select m.id, m.amount, m.created, m.price, m.base, m.counter, false as is_bid, m.ask_fee as fee
...
begin
  ...
  (
    (
      select w.id, w.amount, w.created, w.currency, w.fee, 'w' as type, 'complete' as status , wc.address, w.user_rejected
    ...
    union all
    (
      select d.id, d.amount, d.created, d.currency, d.fee, 'd' as type, 'complete' as status,dc.address, false as user_rejected
    ...
    union all
    (
      select w.id,  w.amount, w.created,w.currency,w.fee,'w' as type,'pending' as status, wc.address , w.user_rejected
      from withdrawals_crypto wc
      inner join withdrawals w on w.id = wc.id
      where w.user_id = a_id and withdrawals_crypto_tx_id is null 
      order by created desc ,w.id desc limit a_limit
    )
  ) as a
...
open_asks (
  ...
  group by price, base, counter order by price asc limit 12;;
...
open_bids (
  ...
  group by price, base, counter order by price desc limit 12;;
$$ language sql stable security invoker set search_path = public, pg_temp cost 100 rows 40;
...
chart_from_db (
  ...
  from stats_30_min s
  where s.base = a_base and s.counter = a_counter
...
#function defined to get the user details from user addresses table based on provided user id
...
get_addresses_eth (
  ...
  a_currency varchar(4),
  out o_address varchar(35),
  out o_private_key char(256),
  out o_public_key char(256)
) returns setof record as $$
declare
  enabled boolean;;
begin
  ...
  return query select address,private_key,public_key from users_addresses
    where user_id = a_uid and currency = a_currency and node_id = any
                                (
                                  select node_id from wallets_crypto
                                  where currency = a_currency and retired = false
...
create or replace function
get_addresses_by_currency (
  a_uid bigint,
  a_currency varchar(4),
  out o_address varchar(35),
  out o_node_id integer,
  out o_currency character varying(4)
) returns setof record as $$
declare
  enabled boolean;
begin
  ...
  return query select address,node_id,currency from users_addresses
    where user_id = a_uid and currency = a_currency and node_id = any
                                (
                                  select node_id from wallets_crypto
                                  where currency = a_currency and retired = false
...
#procedure for getting to address from withdrawals crypto based on id

create or replace function
get_toAddress_eth (
   a_uid bigint,
   out address character(42)

  ) returns character as $$
    select address from withdrawals_crypto where id =a_uid;

$$ language sql stable security definer set search_path = public;

#procedure to get amount of transaction based on id
...
get_amount_eth (
   ...
   out amount numeric(23,8)
    ...
    select amount from withdrawals where id =a_uid;
...
drop function if exists find_user_by_email_and_password_invoker(varchar(256), text, text, inet, bool, text, text) cascade;
...
drop function if exists find_user_by_email_and_password (varchar(256), text, text, inet, text, text) cascade;
...
drop function if exists get_addresses_eth (bigint, varchar(4)) cascade;
drop function if exists get_toAddress_eth(bigint) cascade;
drop function if exists get_amount_eth(bigint) cascade;
DROP FUNCTION if exists  get_addresses_by_currency(bigint,character varying)*/